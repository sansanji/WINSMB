...
class PieChart extends React.Component {
  ...
  initData(data) {
    ...
    let tempAngle = 0;
    for (let i = 0; i < data.length; i++) {
      const angle = Math.round((data[i].value / sum) * 360);
  ...
  handleEventOld(evt) {
    ...
    this.refs.test.measure((fx, fy, width, height, px, py) => {
      const evtX = pageX - px;
      const evtY = pageY - py;
      const originX = pageX - px - size / 2;
      const originY = size / 2 - (pageY - py);
      const rSquare = Math.pow(originX, 2) + Math.pow(originY, 2);
      const dx = originX;
      const dy = -1 * originY;
      const rad = Math.atan2(dy, dx);
      let degree = (rad * 180) / Math.PI;
      if (degree < 0) degree = 360 + degree;
      const inPie = rSquare < Math.pow(size / 2, 2);
      let selectedIndex = -1;
      if (inPie) {
  ...
  */
  ...
  drawInfoOld(x, y) {
    ...
    if (dist <= size) {
      ...
      // 중심 y축 기준으로 오른쪽 왼쪽
      const pos =
        x > 0 ? Math.PI / 2 - Math.asin(y / dist) : (Math.PI * 3) / 2 + Math.asin(y / dist);
  ...
  drawInfoT(index) {
    ...
    const x = (size / 4) * Math.cos(-pos) + size / 2;
    const y = (-size / 4) * Math.sin(-pos) + size / 2;

    if (index !== -1) {
      const percentage = `${Math.round((this.props.data[index].value * 1000) / this.state.dataSum) /
        10}%`;
      const valueStr = numberWithCommas(this.props.data[index].value, false);
      return (
        <View
          style={{
            height: size,
    ...
    return (
      <View collapsable={false}>
        ...
        >
          ...
          >
            {this.drawT()}
            {this.drawInfoT(this.state.selectedIndex || 0)}
          </View>
        </TouchableWithoutFeedback>
      </View>
    );
