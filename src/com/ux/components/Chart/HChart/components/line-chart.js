...
class LineChart extends React.Component {
  ...
  getTransform(rad, width) {
    const x = (0 - width / 2) * Math.cos(rad) - (0 - width / 2) * Math.sin(rad);
    const y = (0 - width / 2) * Math.sin(rad) + (0 - width / 2) * Math.cos(rad);
  ...
  drawCoordinate(
    ...
    const dx = end.gap - start.gap;
    const dy = end.ratioY - start.ratioY;
    const size = Math.sqrt(dx * dx + dy * dy);
    const angleRad = -1 * Math.atan2(dy, dx);
    let height;
    let top;
    const topMargin = 20;
    ...
    return (
      ...
      >
        ...
        {seriesIndex === this.state.sortedData.length - 1 && (
          <TouchableWithoutFeedback
            onPress={() => {
              const selectedIndex = lastCoordinate ? index - 1 : index;
  ...
  drawCoordinates(data, seriesColor, seriesIndex) {
    ...
    const lastData = Object.assign({}, data[dataLength - 1]);
    const lastCoordinate = Object.assign({}, data[dataLength - 1]);
    lastCoordinate.gap += this.props.gap;
    result.push(
      this.drawCoordinate(
        dataLength,
        lastData,
  ...
  drawSelected(index) {
    ...
    const data = this.state.sortedData[0].data;
    ...
    if (typeof this.state.selectedIndex === 'number' && this.state.selectedIndex >= 0) {
      ...
      let reverse = true;
      const bottom = dataObject.ratioY;
      ...
      return (
        ...
        >
          ...
          <View style={StyleSheet.flatten([styles.selectedBox])}>
            {this.state.sortedData.map(series => {
              const dataObject = series.data[this.state.selectedIndex];
              return (
                <View key={series.seriesName}>
                  {dataObject.x ? <Text style={styles.tooltipTitle}>{dataObject.x}</Text> : null}
