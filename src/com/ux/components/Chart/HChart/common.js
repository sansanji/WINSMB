    ...
    for (let i = 0; i < length; i++) {
      ...
      if (typeof dataProp[i] === 'number') {
        ...
        if (dataProp[i].y === null) {
          ...
          dataProp[i].y =
            dataProp[i - 1].y + (dataProp[i + nullCount].y - dataProp[i - 1].y) / (nullCount + 1);
          isEmpty = true;
...
export const getGuideArray = (max, height, numberOfPoints = 5) => {
  ...
  const first = parseInt(x.toString()[0]);
    ...
    {arr.length === 0 ? (
      ...
      arr.map((v, i) => {
        ...
        return (
          ...
          >
            <Text style={{ fontSize: 11, color }}>{v[0]}</Text>
          </View>
        );
      })
    )}
