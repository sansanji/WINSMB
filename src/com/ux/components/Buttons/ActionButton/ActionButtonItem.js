...
export default class ActionButtonItem extends Component {
  ...
  _renderTitle() {
    ...
    const offsetTop = Math.max((size - size / 10) / 2 - TEXT_HEIGHT / 2, 0);
    ...
    if (position !== 'center') {
      positionStyles[position] =
        offsetX +
        (parentSize - (size - size / 10)) / 2 +
        (size - size / 10) +
        spaceBetween -
        size / 2.2; // original : - size / 2.2 없음
    } else {
      positionStyles.right = WIDTH / 2 + (size - size / 10) / 2 + spaceBetween;
    }
  ...
  render() {
    ...
    const buttonStyle = {
      ...
      width: size - size / 10,
      height: size - size / 10,
      borderRadius: (size - size / 10) / 2,
      backgroundColor: this.props.buttonColor || this.props.btnColor,
    };

    if (position !== 'center') {
      buttonStyle[position] = (this.props.parentSize - (size - size / 10)) / 2;
    }
...
const styles = StyleSheet.create({
  textContainer: {
    ...
    // borderColor: '#eee',
    backgroundColor: bluecolor.basicWhiteColor, // 'white',
    height: TEXT_HEIGHT,
  },
  text: {
    flex: 1,
    fontSize: 11,
    color: bluecolor.basicBlueFontColor, // '#444',
  },
});
