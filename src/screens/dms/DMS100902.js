...
class Component extends NavigationScreen {
  ...
  componentWillMount() {
    modelUtil.setModelData('DMS100902_P', {
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('DMS030510SVC', 'getDT', {
      body: JSON.stringify({
        DMS030510F1: modelUtil.getModelData('DMS100902_P'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
      // 모델에 데이터를 set해주면 모델을 쓸수 있다.
      // modelUtil.setModelData('DMS100902', result.DMS030510G2[0]);
      this.setState(
        {
