...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode) {
      modelUtil.setModelData('DMS100502', {
        WH_CODE: whCode,
  ...
  async fetch(sort, callback) {
    ...
    const result = await Fetch.request('DMS030511SVC', 'getStockAll', {
      body: JSON.stringify({
        DMS030511F1: modelUtil.getModelData('DMS100502'),
      }),
    });
    if (result) {
      this.setState(
        {
          dataTotal: result.DMS030511G6,
          data: result.DMS030511G6,
          status: {
            TYPE: result.TYPE,
            MSG: result.MSG,
          },
  ...
  renderBody = (item, index) => (
    <HFormView style={{ marginTop: 2 }}>
      ...
      <HRow style={{ flex: 1 }} between>
        <HText value={item.VENDOR_NAME} textStyle={{ fontSize: 12 }} />
      </HRow>
      <HRow between>
