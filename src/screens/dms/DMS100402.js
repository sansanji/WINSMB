...
import { _, React, Util, Redux, Fetch, NavigationScreen, Navigation, modelUtil, env, bluecolor } from 'libs';
import {
  HBaseView,
  HRow,
  HDateSet,
...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode) {
      modelUtil.setModelData('DMS100402', {
        WH_CODE: whCode,
  ...
  async fetch(sort) {
    ...
    if (!Util.isEmpty(sort)) {
      modelUtil.setValue('DMS100402.M_KEYWORD', sort);
    } else {
      modelUtil.setValue('DMS100402.M_KEYWORD', this.state.M_KEYWORD);
    }


    const result = await Fetch.request('DMS030510SVC', 'getPallet', {
      body: JSON.stringify({
        DMS030510F1: modelUtil.getModelData('DMS100402'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
  ...
  popupItem(item) {
    if (item != null) {
      ...
      const state = modelUtil.getModelData('DMS100402');
      navigator.showOverlay({
        component: {
          name: 'screen.DMS100408', // 변경 필요한지 봐야함.
          passProps: {
            title: 'Item Detail',
            item,
            PLT_YN: 'Y',
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'DMS100402.GR_DATE_FROM',
          TO_DATE: 'DMS100402.GR_DATE_TO',
          DATE_TYPE: 'DMS100402.DATE_TYPE',
        }}
        fDateNum={-3}
        tDateNum={0}
      />
      <HRow>
        <HCheckbox label={'Exist'} bind={'DMS100402.EXIST_YN'} editable />
        <HCheckbox label={'BOX'} bind={'DMS100402.BOX_YN'} toggle editable />
        <HCheckbox label={'ITEM'} bind={'DMS100402.ITEM_YN'} toggle editable />
      </HRow>
      <TextInput
    ...
    >
      <HFormView style={{ marginTop: 2 }}>
        ...
        <HRow>
          ...
          <HNumberfield label={'Box'} value={item.BOX_QTY} />
