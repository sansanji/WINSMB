...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode) {
      modelUtil.setModelData('DMS100301', {
        COMPANY_CODE,
        WH_CODE: whCode,
        VENDOR_CODE: vendorCode,
        GR_DATE_FROM: Util.getDateValue(null, -10),
  ...
  async fetch(sort, callback) {
    ...
    if (!Util.isEmpty(sort)) {
      modelUtil.setValue('DMS100301.M_KEYWORD', sort);
    } else {
      modelUtil.setValue('DMS100301.M_KEYWORD', this.state.barcodeData1);
    }


    const result = await Fetch.request('DMS030050SVC', 'get', {
      body: JSON.stringify({
        DMS030050F1: modelUtil.getModelData('DMS100301'),
      }),
    });
    if (result.TYPE === 1) {
      // 정해진 데이터만 보여준다.
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'DMS100301.GR_DATE_FROM',
          TO_DATE: 'DMS100301.GR_DATE_TO',
          DATE_TYPE: 'DMS100301.DATE_TYPE',
        }}
        fDateNum={-3}
        tDateNum={0}
      />
