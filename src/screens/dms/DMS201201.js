...
class Component extends NavigationScreen {
  ...
  componentWillMount() {
    ...
    if (whCode) {
      modelUtil.setModelData('DMS201201', {
  ...
  async fetch(callback, seqno) {
    const reqData = modelUtil.getModelData('DMS201201');
    if (seqno) {
  ...
  focusNextField(scanData) {
    ...
    for (let i = 0; i < dataLength; i += 1) {
      ...
      if (g1ScanData === barcode1Data) {
        if (this.state.totalData[i].CHECK_YN === 'Y') {
          // 이미 스캔 된건이 중복 스캔 되는 경우
          this.setState({
            scanVaildData: `"${barcode1Data}" already checking complete`,
            barcodeScanData: null,
            barcodeScanIndex: null,
          });
    ...
    if (barcodeYN === 'Y') {
      ...
      if (TOTAL_QTY === CHECK_QTY) {
        this.setState({
          ...
          status: {
            MSG: `스캔완료(${listTotalCount}/${dataLength})`,
          },
        });
  ...
  _onPress(item) {
    ...
    if (requestParam) {
      ...
      requestParam = {
        ...modelUtil.getModelData('DMS201201'),
        SCAN_NO: scanData,
      };
  ...
  async fetchCreate(reqData) {
    ...
    const data = modelUtil.getModelData('DMS201201');
    ...
    // const STK_DATE = modelUtil.getValue('DMS201201.STK_DATE');
    const whCode = _.get(this.props.global, 'dmsWhcode.WH_CODE', null);
    ...
    const result = await Fetch.request('DMS030510SVC', 'insertStock', {
      body: JSON.stringify({
        DMS030510F1: {
          ...
          CHECK_COUNT: reqData.CHECK_COUNT,
          REMARKS: 'RANDOM',
        },
      }),
    });
        ...
        buttonGroup: [
          {
            ...
            onPress: () => {
              Util.openLoader(this.screenId, false);
              this.fetch(() => {
                modelUtil.setValue('DMS201201.SEQ_NO', result.data.SEQ_NO);
                modelUtil.setValue('DMS201201.SEQ_NO_NAME', result.data.SEQ_NO);
              }, result.data.SEQ_NO);
            },
          },
        ],
  ...
  async fetchConfirm(AUTO_YN) {
    Util.openLoader(this.screenId, true); // Loader View 열기!
    const SEQ_NO = modelUtil.getValue('DMS201201.SEQ_NO');
    const STK_DATE = modelUtil.getValue('DMS201201.STK_DATE');
    const whCode = _.get(this.props.global, 'dmsWhcode.WH_CODE', null);
    const companyCode = this.props.global.session.COMPANY_CODE;
  ...
  renderHeader = () => (
    <View>
      <HRow>
        <HDatefield label={'Stock Date'} bind={'DMS201201.STK_DATE'} editable rowflex={1.5} />
        <HTextfield
          label={'Area'}
          bind={'DMS201201.AREA'}
          keyboardType="email-address"
          onChanged={() => {
            modelUtil.setValue('DMS201201.SEQ_NO', '');
            modelUtil.setValue('DMS201201.SEQ_NO_NAME', '');
          }}
          editable
        />
      ...
      <HRow>
        ...
        <HTextfield
          label={'Item Code'}
          bind={'DMS201201.CREATE_ITEM_CODE'}
          keyboardType="email-address"
          onChanged={() => {
            modelUtil.setValue('DMS201201.SEQ_NO', '');
            modelUtil.setValue('DMS201201.SEQ_NO_NAME', '');
        ...
        <HTextfield
          label={'Item Name'}
          bind={'DMS201201.CREATE_ITEM_NAME'}
          keyboardType="email-address"
          onChanged={() => {
            modelUtil.setValue('DMS201201.SEQ_NO', '');
            modelUtil.setValue('DMS201201.SEQ_NO_NAME', '');
