...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    // 체크박스 설정 관련
    modelUtil.setModelData('DMS201302', {
  ...
  async fetchList(callback, focus) {
    ...
    const result = await Fetch.request('WMS050315SVC', 'getIn', {
      body: JSON.stringify({
        WMS050315F1: {
          COMPANY_CODE,
          WH_CODE,
    ...
    if (result) {
      // 정해진 데이터만 보여준다.
      const data = Util.getArrayData(result.WMS050315G1, env().listCnt);
      this.setState(
        {
          dataTotal: result.WMS050315G1,
          data,
          status: {
            TYPE: result.TYPE,
            MSG: result.MSG,
  ...
  async saveScan(WH_CODE, BARCODE, TYPE, WEIGHT, callback) {
    Util.openLoader(this.screenId, true); // Loader View 열기!
    const result = await Fetch.request('WMS050315SVC', 'saveIn', {
      body: JSON.stringify({
        WMS050315G1: {
          data: [{
            GR_DATE: this.state.Date,
            BARCODE,
            TYPE,
            WEIGHT,
            WH_CODE,
  ...
  async resetScanValue() {
    const { fixWh, fixLabel, fixType, fixWeight } = modelUtil.getModelData('DMS201302');
    const barSeq = [];
  ...
  return (
    <HBaseView scrollable={false}>
      <View>
        ...
        <View style={styles.spaceAroundStyle}>
          <HCheckbox label={'WH Code fix'} bind={'DMS201302.fixWh'} editable />
          <HCheckbox label={'Label fix'} bind={'DMS201302.fixLabel'} editable />
          <HCheckbox label={'Type fix'} bind={'DMS201302.fixType'} editable />
          <HCheckbox label={'Weight fix'} bind={'DMS201302.fixWeight'} editable />
