...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode) {
      modelUtil.setModelData('DMS100407', {
        WH_CODE: whCode,
  ...
  async fetch(sort) {
    ...
    if (!Util.isEmpty(sort)) {
      modelUtil.setValue('DMS100407.M_KEYWORD', sort);
    } else {
      modelUtil.setValue('DMS100407.M_KEYWORD', this.state.M_KEYWORD);
    }

    const result = await Fetch.request('DMS030510SVC', 'getItem', {
      body: JSON.stringify({
        DMS030510F1: modelUtil.getModelData('DMS100407'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'DMS100407.GR_DATE_FROM',
          TO_DATE: 'DMS100407.GR_DATE_TO',
          DATE_TYPE: 'DMS100407.DATE_TYPE',
        }}
        fDateNum={-3}
        tDateNum={0}
      />

      <HCheckbox label={'Exist'} bind={'DMS100407.EXIST_YN'} editable />
      <TextInput
