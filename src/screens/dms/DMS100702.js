...
class Component extends NavigationScreen {
  ...
  componentWillMount() {
    ...
    if (whCode && vendorCode) {
      modelUtil.setModelData('DMS100702', {
        WH_CODE: whCode,
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('COM080101SVC', 'getDw', {
      body: JSON.stringify({
        COM080101F2: modelUtil.getModelData('DMS100702'),
      }),
    });
    if (result.TYPE === 1) {
      // 정해진 데이터만 보여준다.
      // 모델에 데이터를 set해주면 모델을 쓸수 있다.
      modelUtil.setModelData('DMS100702', result.COM080101G1[0]);
      this.setState(
        {
  ...
  async onSave() {
    ...
    Util.openLoader(this.screenId, false);
    if (result.TYPE === 1) {
      modelUtil.setValue('DMS100702.IRRE_SEQ_NO', result.data.IRRE_SEQ_NO);
      Util.msgBox({
        title: 'Save',
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1 }}>
          <HPhotoView
            style={{ flex: 3.5 }}
            isAttachable={this.state.isAttachable || modelUtil.getValue('DMS100702.IRRE_SEQ_NO')}
            photoList={this.state.photoList}
            REF_NO={modelUtil.getValue('DMS100702.IRRE_SEQ_NO')}
            onSuccess={() => {
              this.fetch();
            }}
          />
