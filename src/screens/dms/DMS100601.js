  ...
  async fetch(barcode1Data) {
    ...
    if (this.state.data != null) {
      ...
      if (this.state.data[0].STK_NO === null || this.state.data[0].STK_NO === '') {
        tabCode = 'GR_REF_DT';
        if (config.DMS_BLOCK_DUP) {
          if (config.DMS_BLOCK_DUP === 'Y') {
            tabCode = 'M_GR_REF_DT';
          }
        }
      } else {
      ...
      if (config.DMS_LOC_FORMAT && changeType === 'LOCATION') {
      ...
      if (config.DMS_LOT_FORMAT && changeType === 'LOT_NO') {
      ...
      const result = await Fetch.request('DMS010212SVC', reqService, {
        body: JSON.stringify({
          DMS010212F1: {
            WH_CODE: whCode,
            TAB_CODE: tabCode,
      ...
      const result2 = await Fetch.request('DMS010212SVC', 'getStockAll', {
        body: JSON.stringify({
          DMS010212F1: {
            WH_CODE: whCode,
      ...
      if (result2) {
        if (result2.DMS010212G6[0]) {
          result2.DMS010212G6[0].SCAN_NO = barcode1Data;
          // 스캔 성공한 경우
          this.setState({
            scanVaildData: `"${result2.DMS010212G6[0].REF_NO}" Scan Success!`,
            VENDOR_NAME: result2.DMS010212G6[0].VENDOR_NAME,
            GR_NO: result2.DMS010212G6[0].GR_NO,
            REF_NO: result2.DMS010212G6[0].REF_NO,
            LOCATION: result2.DMS010212G6[0].LOCATION,
            STOCK_DAYS: result2.DMS010212G6[0].STOCK_DAYS,
            MOVED_LOCATION: null,
            spinner: false,
            data: result2.DMS010212G6,
          });
          this.successScan();
        } else {
          // 스캔 실패한 경우
