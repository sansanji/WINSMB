...
class Component extends NavigationScreen {
    ...
    const result = await Fetch.request('TMS010101SVC', 'getSimpleTRNListOC', {
      body: JSON.stringify({
        TMS010101F1: {
          ...
          CAR_NO: this.props.global.session.USER_ID,
        },
      }),
    });
  ...
  async onSave() {
    ...
    if (location && beacon.apiCode === 'MB') {
      ...
      const result = await Fetch.request('TMS010142SVC', 'savePositionWithoutTRNOC', {
        body: JSON.stringify({
          TMS010142F1: {
      ...
      if (result) {
        if (result.TYPE === 1) {
          ...
          console.log('savePositionWithoutTRNOC', result);
    ...
    return (
      <View style={styles.stepButtonContainer}>
        ...
        <View style={styles.stepButtonInnerConatainer}>
          ...
          >
            <View style={[styles.stepButton, { backgroundColor: this.state.carStatus === 'W' ? '#d84329cf' : '#559854c7' }]}>
              <HText
