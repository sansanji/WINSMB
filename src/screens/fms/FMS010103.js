...
class Component extends NavigationScreen {
  ...
  componentWillMount() {
    ...
    modelUtil.setModelData('FMS010103', {
      H_CONFIRM_YN: '',
      ONLY_QUE_CHK: 'N',
      USER_ID_CHK: 'N',
  ...
  async fetch() {
    ...
    modelUtil.setValue(
      'FMS010103.HMBL_NO',
      modelUtil
        .getValue('FMS010103.HMBL_NO')
        .toString()
        .toUpperCase(),
    );
    modelUtil.setValue(
      'FMS010103.ARRIVAL_PORT',
      modelUtil
        .getValue('FMS010103.ARRIVAL_PORT')
        .toString()
        .toUpperCase(),
    );
    ...
    const result = await Fetch.request(
      ...
      {
        body: JSON.stringify({
          BAS010101F1: modelUtil.getModelData('FMS010103'),
        }),
      },
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        label={'Shipping'}
        bindVar={{
          FROM_DATE: 'FMS010103.FROM_DATE',
          TO_DATE: 'FMS010103.TO_DATE',
          DATE_TYPE: 'FMS010103.DATE_TYPE',
        }}
        lDateNum={3}
        fDateNum={-1}
        tDateNum={0}
      />
      <HRow>
        <HTextfield label={'Arrival Port'} bind={'FMS010103.ARRIVAL_PORT'} editable />
        <HTextfield label={'H/M BL No.'} bind={'FMS010103.HMBL_NO'} editable />
        <HCombobox
          label={'EDI Status'}
          groupCode={'EDI_SEND_STATUS'}
          bindVar={{
            CD: 'FMS010103.SEND_TARGET_STATUS',
            NM: 'FMS010103.SEND_TARGET_STATUS_NAME',
          }}
          editable
        />
      </HRow>
