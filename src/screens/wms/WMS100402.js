...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode && vendorPlantCode) {
      modelUtil.setModelData('WMS100402', {
        WH_CODE: whCode,
  ...
  async fetch() {
    ...
    const result = await Fetch.request('WMS010212SVC', 'getRef', {
      body: JSON.stringify({
        WMS010212F1: modelUtil.getModelData('WMS100402'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
      const data = Util.getArrayData(result.WMS010212G2, env().listCnt);
      this.setState({
        dataTotal: result.WMS010212G2,
        data,
        status: {
          TYPE: result.TYPE,
          MSG: result.MSG,
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'WMS100402.GR_DATE_FROM',
          TO_DATE: 'WMS100402.GR_DATE_TO',
          DATE_TYPE: 'WMS100402.DATE_TYPE',
        }}
        fDateNum={-3}
        tDateNum={0}
      />
      <HRow>
        <HTextfield label={'Search'} bind={'WMS100402.M_KEYWORD'} editable />
        <HCheckbox label={'Exist'} bind={'WMS100402.EXIST_YN'} editable />
      </HRow>
    </View>
  );
