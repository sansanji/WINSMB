...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode && vendorPlantCode) {
      modelUtil.setModelData('WMS100701', {
        WH_CODE: whCode,
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('COM080101SVC', 'getWm', {
      body: JSON.stringify({
        COM080101F2: modelUtil.getModelData('WMS100701'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'WMS100701.ISSUE_DATE_FROM',
          TO_DATE: 'WMS100701.ISSUE_DATE_TO',
          DATE_TYPE: 'WMS100701.DATE_TYPE',
        }}
        fDateNum={-7}
        tDateNum={0}
      />
      <HTextfield label={'Search'} bind={'WMS100701.REF_NO'} editable />
    </View>
  );
