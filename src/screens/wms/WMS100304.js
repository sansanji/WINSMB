...
class Component extends NavigationScreen {
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('WMS010205SVC', 'getDt', {
      body: JSON.stringify({
        WMS010205F1: {
  ...
  async focusNextField(targetType, scanData) {
    ...
    if (locationTarget === 'Y') {
      ...
      if (targetType === 'itemField') {
        ...
        for (let i = 0; i < dataLength; i += 1) {
          ...
          if (config.WMS_SIMPLE_YN) {
            if (config.WMS_SIMPLE_YN === 'Y') {
              scanNo = this.state.data[i].REF_NO.toUpperCase().trim();
            }
          }
        ...
        if (Util.checkBarcode(barcode2Data) === 'REF') {
          // length check
  ...
  _finalCheckDisplay(barcodeYN, currentIndex, barcode1Data, barcode2Data, locationSaveYN) {
    ...
    const targetG1ScanData =
      currentIndex !== null ? this.state.data[currentIndex].REF_NO.trim() : barcode1Data;
    ...
    >
      ...
      >
        ...
        <HRow>
          ...
          <HNumberfield label={'W/t:'} value={item.WT} />
        </HRow>
        <HRow between>
          <HText value={`${item.ZONE_CODE} / ${item.LOCATION}`} />
