...
class Component extends NavigationScreen {
  ...
  componentWillMount() {
    modelUtil.setModelData('WMS110301', {
      WH_CODE: 'WH09',
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('WMS020121SVC', 'getOrder', {
      body: JSON.stringify({
        WMS020121F1: modelUtil.getModelData('WMS110301'),
      }),
    });
    ...
    let count = 0;
    let ingcount = 0;
    for (let i = 0; i < result.WMS020121G1.length; i += 1) {
      if (result.WMS020121G1[i].CHECK_QTY === result.WMS020121G1[i].GI_PKG_QTY) {
        count += 1;
      } else if (result.WMS020121G1[i].CHECK_QTY > 0) {
        ingcount += 1;
      }
    }
    ...
    if (result) {
      // 정해진 데이터만 보여준다.
      // const data = Util.getArrayData(result.WMS020121G1, env().listCnt);
      this.setState(
        {
          totalData: result.WMS020121G1,
          data: result.WMS020121G1,
          status: {
            TYPE: result.TYPE,
  ...
  filterSearch(text, target) {
    const dataList = Util.filterData(this.state.totalData, target, text);
    if (target === 'BL_NO') {
      this.setState({
        filterText: text,
        data: dataList,
  ...
  focusNextField(scanData) {
    ...
    for (let i = 0; i < dataLength; i += 1) {
      ...
      g1ScanData = this.state.totalData[i].BL_NO.toUpperCase().trim();
      if (barcode1Data === g1ScanData) {
        ...
        TOTAL_QTY = dataList[currentIndex].GI_PKG_QTY;
      }
    }
    ...
    if (barcodeYN === 'Y') {
      ...
      targetG1ScanData = this.state.totalData[currentIndex].BL_NO.trim();
  ...
  _onCheckPress(item, index) {
    ...
    let setScanQty = item.GI_PKG_QTY;
    if (item.CHECK_QTY === item.GI_PKG_QTY) {
      setScanQty = 0;
    }
    totalData.forEach(x => {
      if (x === item) {
  ...
  onConfirm() {
    ...
    for (let i = 0; i < dataLength; i += 1) {
      if (dataList[i].CHECK_QTY) {
        if (dataList[i].CHECK_QTY === dataList[i].GI_PKG_QTY) {
    ...
    console.log(reqList);
    let msg = '반출처리 및 반출EDI 전송을 하시겠습니까?';
  ...
  async fetchConfirm(reqList) {
    Util.openLoader(this.screenId, true); // Loader View 열기!
    const result = await Fetch.request('WMS020121SVC', 'sendMobileCarry', {
      body: JSON.stringify({
        WMS020121F1: modelUtil.getModelData('WMS110301'),
        WMS020121G1: {
          data: reqList,
  ...
  async fetchSave(reqList) {
    ...
    const result = await Fetch.request('WMS020121SVC', 'checkSave', {
      body: JSON.stringify({
        WMS020121F1: modelUtil.getModelData('WMS110301'),
        WMS020121G1: {
          data: reqList,
  ...
  renderHeader = () => (
    <View>
      <HRow>
        <HDateSet
          bindVar={{
            FROM_DATE: 'WMS110301.FROM_DATE',
            TO_DATE: 'WMS110301.TO_DATE',
            DATE_TYPE: 'WMS110301.DATE_TYPE',
          }}
          fDateNum={-7}
          tDateNum={1}
          rowflex={5}
        ...
        <HCombobox
          ...
          groupCode={'USE_YN'}
          bindVar={{
            CD: 'WMS110301.WH_GI_YN',
            NM: 'WMS110301.WH_GI_YN_NAME',
          }}
          editable
          rowflex={1}
        />
      ...
      <View style={styles.filterInput}>
        <TextInput
          ...
          onChangeText={filterCar => this.filterSearch(filterCar, 'CAR_NO')}
      ...
      <View style={styles.filterInput}>
        <TextInput
          ...
          onChangeText={filterText => this.filterSearch(filterText, 'BL_NO')}
  ...
  renderBody = (item, index, scanData, scanIndex) => (
    <View style={{ flex: 1 }} key={item.GI_REF_NO}>
      <HFormView
        style={[
          { marginTop: 2 },
          item.CHECK_QTY === item.GI_PKG_QTY
            ? { backgroundColor: bluecolor.basicSkyLightBlueColor }
            : null,
          item.WH_GI_YN === 'D' ? { backgroundColor: bluecolor.basicGreenColor } : null,
        ]}
      >
        <HRow>
          <Touchable rowflex={1.3} onPress={() => this._onCountPress(item, index)}>
            <HText value={item.BL_NO} textStyle={{ fontWeight: 'bold' }} />
          </Touchable>
          <HText
            value={`${item.GI_PKG_QTY}/${item.GR_REMAIN_PKG_QTY}`}
            rowflex={0.6}
            textStyle={[
              item.GI_PKG_QTY !== item.GR_REMAIN_PKG_QTY
                ? { color: bluecolor.basicOrangeColor, textAlign: 'right' }
                : { textAlign: 'right' },
            ]}
          />
          <HText
            rowflex={1}
            value={`${item.GI_GROSS_WEIGHT}/${item.GR_REMAIN_WEIGHT}`}
            textStyle={[
              item.GI_GROSS_WEIGHT !== item.GR_REMAIN_WEIGHT
                ? { color: bluecolor.basicOrangeColor, textAlign: 'right' }
                : { textAlign: 'right' },
            ]}
          />
          <HText value={item.LOCATION} rowflex={0.4} />
          <HButton
          ...
          <HText
            ...
            value={`${item.CHECK_QTY || 0}/${item.GI_PKG_QTY}`}
            rowflex={0.5}
          />
        </HRow>
      </HFormView>
  ...
  render() {
    ...
    return (
      <HBaseView style={styles.container} scrollable={false} buttonGroup={buttonGroup}>
        ...
        <HListView
          keyExtractor={item => item.GI_REF_NO}
