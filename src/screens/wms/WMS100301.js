...
class Component extends NavigationScreen {
  ...
  async componentWillMount() {
    ...
    if (whCode && vendorCode && vendorPlantCode) {
      modelUtil.setModelData('WMS100301', {
        WH_CODE: whCode,
  ...
  async fetch(sort, callback) {
    ...
    const result = await Fetch.request('WMS010205SVC', 'get', {
      body: JSON.stringify({
        WMS010205F1: modelUtil.getModelData('WMS100301'),
      }),
    });
    if (result) {
      // 정해진 데이터만 보여준다.
      const data = Util.getArrayData(result.WMS010205G1, env().listCnt);
      this.setState(
        {
          dataTotal: result.WMS010205G1,
          data,
          status: {
            TYPE: result.TYPE,
            MSG: result.MSG,
  ...
  renderHeader = () => (
    <View>
      <HDateSet
        bindVar={{
          FROM_DATE: 'WMS100301.GR_DATE_FROM',
          TO_DATE: 'WMS100301.GR_DATE_TO',
          DATE_TYPE: 'WMS100301.DATE_TYPE',
        }}
        fDateNum={-3}
        tDateNum={0}
      />
      <HRow>
        <HTextfield label={'Search'} bind={'WMS100301.M_KEYWORD'} editable />
        <HCheckbox
          label={langUtil.get(this, 'A000000001')}
          bind={'WMS100301.M_GR_STATUS'}
          editable
        />
      </HRow>
    </View>
