...
class Component extends NavigationScreen {
  ...
  async fetch(callback) {
    ...
    const result = await Fetch.request('WMS010209SVC', 'getDt', {
      body: JSON.stringify({
        WMS010209F1: {
    ...
    >
      ...
      >
        <HRow between>
          ...
          />
          <HText value={`${item.GR_NO}(${item.GR_SEQ_NO})`} />
        </HRow>
        <HRow>
          <HNumberfield label={'Item'} value={item.GI_ITEM_QTY} />
          <HNumberfield label={'C/T'} value={item.GI_CT_QTY} />
          <HNumberfield label={'Plt'} value={item.GI_PLT_QTY} />
          <HNumberfield label={'W/t:'} value={item.GI_WT} />
        </HRow>
        <HRow between>
  ...
  render() {
    return (
      <HBaseView style={styles.container} scrollable={false}>
        ...
        <HListView
          keyExtractor={item => item.GI_SEQ_NO}
          renderHeader={null}
